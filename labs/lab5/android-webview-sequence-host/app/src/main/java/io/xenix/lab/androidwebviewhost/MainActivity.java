...
public class MainActivity extends AppCompatActivity {
    ...
    private final class ProbeBridge {
        ...
        public void onResult(String detail) {
            events.add(detail);
            if ("done".equals(detail)) {
                String[] expected = new String[] {"boot", "ready", "result:3", "done"};
                java.util.List<String> expectedList = java.util.Arrays.asList(expected);
                if (events.equals(expectedList)) {
                    writeProbeResult("webview-sequence-probe.json", "{\"status\":\"ok\",\"detail\":\"" + escapeJson(String.join(" -> ", events)) + "\"}");
                } else {
                    writeProbeResult("webview-sequence-probe.json", "{\"status\":\"fail\",\"detail\":\"" + escapeJson("expected " + String.join(" -> ", expectedList) + " got " + String.join(" -> ", events)) + "\"}");
                }
            }
        }
    }
