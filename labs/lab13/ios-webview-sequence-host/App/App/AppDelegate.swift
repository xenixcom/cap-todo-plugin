...
private final class ProbeViewController: UIViewController, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    override func viewDidLoad() {
        ...
        guard let resourceURL = Bundle.main.resourceURL else {
            resultWriter.write(fileName: "wkwebview-bundled-probe.json", status: "error", detail: "missing bundle resource url")
            return
        }
        ...
        guard FileManager.default.fileExists(atPath: htmlURL.path) else {
            resultWriter.write(fileName: "wkwebview-bundled-probe.json", status: "error", detail: "missing bundled probe assets")
            return
        }
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        if detail == "closed" {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.15) { [weak self] in
                guard let self else { return }
                guard self.closeObserved, !self.reported else { return }
                let expected = ["boot", "open", "data:1", "data:2", "data:3", "closed"]

                if self.events == expected {
                    self.write(status: "ok", detail: self.events.joined(separator: " -> "))
                } else {
                    self.write(status: "fail", detail: "expected \(expected.joined(separator: " -> ")) got \(self.events.joined(separator: " -> "))")
                }
            }
        }
    }
    ...
    private func write(status: String, detail: String) {
        ...
        reported = true
        resultWriter.write(fileName: "wkwebview-stream-probe.json", status: status, detail: detail)
    }
}
