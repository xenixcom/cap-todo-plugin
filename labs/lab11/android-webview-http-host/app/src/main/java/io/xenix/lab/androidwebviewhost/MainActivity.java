    ...
    private final class ProbeBridge {
        ...
        public void onResult(String detail) {
            if ("pass".equals(detail)) {
                writeProbeResult("webview-mock-pressure-probe.json", "{\"status\":\"ok\",\"detail\":\"" + escapeJson(detail) + "\"}");
            } else {
                writeProbeResult("webview-mock-pressure-probe.json", "{\"status\":\"fail\",\"detail\":\"" + escapeJson(detail) + "\"}");
            }
        }
    }
