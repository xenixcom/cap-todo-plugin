...
public class MainActivity extends AppCompatActivity {
    ...
    private String buildTestHtml(String probeMode) {
        ...
        return "<!doctype html><html><head><meta charset=\"utf-8\"></head><body><script>" +
            "window.__test__={addAsync:function(a,b){return new Promise(function(resolve){setTimeout(function(){resolve((function(a,b){" + addBody + "})(a,b));},150);});}};" +
            "window.addEventListener('load', async function(){ window.AndroidProbe.onResult(String(await window.__test__.addAsync(1,2))); });" +
            "</script></body></html>";
    }
    ...
    private final class ProbeBridge {
        ...
        public void onResult(String detail) {
            if ("3".equals(detail)) {
                writeProbeResult("webview-async-probe.json", "{\"status\":\"ok\",\"detail\":\"" + escapeJson(detail) + "\"}");
            } else {
                writeProbeResult("webview-async-probe.json", "{\"status\":\"fail\",\"detail\":\"" + escapeJson("expected 3 got " + detail) + "\"}");
            }
        }
    }
