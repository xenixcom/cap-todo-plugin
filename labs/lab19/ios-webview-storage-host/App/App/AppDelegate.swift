...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(
        ...
        let probeMode = ProcessInfo.processInfo.environment["PROBE_MODE"]
...
private final class ProbeViewController: UIViewController, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    override func viewDidLoad() {
        ...
        guard let resourceURL = Bundle.main.resourceURL else {
            resultWriter.write(fileName: "wkwebview-storage-deeper-probe.json", status: "error", detail: "missing bundle resource url")
            return
        }
        ...
        switch probeMode {
        case "verify":
            assetName = "probe-verify.html"
        case "corrupt":
            assetName = "probe-corrupt.html"
        default:
            assetName = "probe.html"
        }
        let htmlURL = publicURL.appendingPathComponent(assetName)

        guard FileManager.default.fileExists(atPath: htmlURL.path) else {
            resultWriter.write(fileName: "wkwebview-storage-deeper-probe.json", status: "error", detail: "missing bundled probe assets")
            return
        }
    ...
    private func write(status: String, detail: String) {
        ...
        reported = true
        resultWriter.write(fileName: "wkwebview-storage-deeper-probe.json", status: status, detail: detail)
    }
}
