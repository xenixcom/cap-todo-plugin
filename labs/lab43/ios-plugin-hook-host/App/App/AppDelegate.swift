...
private final class ProbeViewController: UIViewController, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    private lazy var webView: WKWebView = {
        ...
        let fixture = probeMode == "fault"
            ? #"{"permission":{"microphone":"denied"},"availability":{"enabled":false},"session":{"openAllowed":false}}"#
            : #"{"permission":{"microphone":"granted"},"availability":{"enabled":true},"session":{"openAllowed":true}}"#
        let userScript = WKUserScript(
            source: "window.__nativeFixture__ = \(fixture);",
    ...
    override func viewDidLoad() {
        ...
        guard let resourceURL = Bundle.main.resourceURL else {
            resultWriter.write(fileName: "wkwebview-native-fixture-probe.json", status: "error", detail: "missing bundle resource url")
            return
        }
        ...
        guard FileManager.default.fileExists(atPath: htmlURL.path) else {
            resultWriter.write(fileName: "wkwebview-native-fixture-probe.json", status: "error", detail: "missing bundled probe assets")
            return
        }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) { [weak self] in
            ...
            JSON.stringify({
              ...
              nativeDone: window.__nativeDone__ === true,
              nativeEvents: window.__nativeEventLog__ ?? []
            })
            """) { value, _ in
                let detail = (value as? String) ?? #"{"href":"unknown"}"#
                self.write(status: "error", detail: "no pushed result after didFinish: \(detail)")
            }
        }
    }

    private func scheduleNativeProgression() {
        webView.evaluateJavaScript("""
    ...
    private func write(status: String, detail: String) {
        ...
        reported = true
        resultWriter.write(fileName: "wkwebview-native-fixture-probe.json", status: status, detail: detail)
    }
}
