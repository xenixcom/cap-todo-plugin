...
public class MainActivity extends AppCompatActivity {
    ...
    private final class ProbeBridge {
        ...
        public void onResult(String detail) {
            if ("3".equals(detail)) {
                writeProbeResult("webview-bundled-probe.json", "{\"status\":\"ok\",\"detail\":\"" + escapeJson(detail) + "\"}");
            } else {
                writeProbeResult("webview-bundled-probe.json", "{\"status\":\"fail\",\"detail\":\"" + escapeJson("expected 3 got " + detail) + "\"}");
            }
        }
    }
