...
private final class ProbeViewController: UIViewController, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        if detail == "done" {
            let expected = ["boot", "ready", "result:3", "done"]

            if events == expected {
                write(status: "ok", detail: events.joined(separator: " -> "))
            } else {
                write(status: "fail", detail: "expected \(expected.joined(separator: " -> ")) got \(events.joined(separator: " -> "))")
            }
        }
    }
    ...
    private func write(status: String, detail: String) {
        ...
        reported = true
        resultWriter.write(fileName: "wkwebview-sequence-probe.json", status: status, detail: detail)
    }
}
