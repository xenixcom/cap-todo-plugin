...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        switch (probeMode) {
            case "verify":
                asset = "probe-verify.html";
                break;
            case "corrupt":
                asset = "probe-corrupt.html";
                break;
            default:
                asset = "probe.html";
                break;
    ...
    private final class ProbeBridge {
        ...
        public void onResult(String detail) {
            if ("pass".equals(detail)) {
                writeProbeResult("webview-storage-deeper-probe.json", "{\"status\":\"ok\",\"detail\":\"" + escapeJson(detail) + "\"}");
            } else {
                writeProbeResult("webview-storage-deeper-probe.json", "{\"status\":\"fail\",\"detail\":\"" + escapeJson(detail) + "\"}");
            }
        }
    }
