    ...
    private final class ProbeBridge {
        ...
        public void onResult(String detail) {
            if ("pass".equals(detail)) {
                writeProbeResult("webview-storage-probe.json", "{\"status\":\"ok\",\"detail\":\"" + escapeJson(detail) + "\"}");
            } else {
                writeProbeResult("webview-storage-probe.json", "{\"status\":\"fail\",\"detail\":\"" + escapeJson(detail) + "\"}");
            }
        }
    }
