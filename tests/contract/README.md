...
- `tools/test-plugin.sh` 是唯一正式測試工具入口，負責調度這裡的測試。
...
- 目前 `./tools/test-plugin.sh all --report` 已可得到三平台全綠結果，但 `ios / android` 仍是以原生 coverage 承接，尚未完全提升到與 `web` 同層級的 app-level pipeline host
