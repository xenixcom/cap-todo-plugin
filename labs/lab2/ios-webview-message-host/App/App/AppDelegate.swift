...
private final class ProbeViewController: UIViewController, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        if detail == "3" {
            resultWriter.write(fileName: "wkwebview-message-probe.json", status: "ok", detail: detail)
        } else {
            resultWriter.write(fileName: "wkwebview-message-probe.json", status: "fail", detail: "expected 3 got \(detail)")
        }
    }
          ...
          <body>
            <script>
              ...
              window.addEventListener('load', () => {
                const result = window.__test__.add(1, 2);
                window.webkit.messageHandlers.probe.postMessage({ detail: String(result) });
              });
            </script>
          </body>
        </html>
