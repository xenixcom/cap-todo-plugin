...
async function runNormal() {
  ...
  const hostMethods = {
    __hostPermissionBefore: () => window.AndroidProbe?.hostPermissionState?.(),
    __hostPermissionAfter: () => window.AndroidProbe?.hostPermissionState?.(),
  };

  for (const testCase of window.__cases__) {
    const fn = hostMethods[testCase.method] ?? hook?.[testCase.method];
