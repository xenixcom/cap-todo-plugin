...
private final class ProbeViewController: UIViewController, WKNavigationDelegate, WKScriptMessageHandler, WKUIDelegate {
    ...
    override func viewDidLoad() {
        ...
        guard let resourceURL = Bundle.main.resourceURL else {
            resultWriter.write(fileName: "wkwebview-plugin-permission-request-probe.json", status: "error", detail: "missing bundle resource url")
            return
        }
        ...
        guard FileManager.default.fileExists(atPath: htmlURL.path) else {
            resultWriter.write(fileName: "wkwebview-plugin-permission-request-probe.json", status: "error", detail: "missing bundled probe assets")
            return
        }
    ...
    func webView(
        ...
        decisionHandler(probeMode == "deny" ? .deny : .grant)
    }
    ...
    private func write(status: String, detail: String) {
        ...
        reported = true
        resultWriter.write(fileName: "wkwebview-plugin-permission-request-probe.json", status: status, detail: detail)
    }
}
