...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(
        ...
        let controller = ProbeViewController(resultWriter: resultWriter, probeMode: probeMode, lifecycleRecorder: lifecycleRecorder)
...
@MainActor
private final class ProbeViewController: UIViewController, WKNavigationDelegate, WKScriptMessageHandler, WKUIDelegate {
    private let resultWriter: ProbeResultWriter
    ...
    init(resultWriter: ProbeResultWriter, probeMode: String, lifecycleRecorder: LifecycleRecorder) {
    ...
    override func viewDidLoad() {
        ...
        guard let resourceURL = Bundle.main.resourceURL else {
            resultWriter.write(fileName: "wkwebview-getusermedia-timer-probe.json", status: "error", detail: "missing bundle resource url")
            return
        }
        ...
        guard FileManager.default.fileExists(atPath: htmlURL.path) else {
            resultWriter.write(fileName: "wkwebview-getusermedia-timer-probe.json", status: "error", detail: "missing bundled probe assets")
            return
        }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        """) { [weak self] value, error in
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 5.0) {
                if self.reported { return }
                webView.evaluateJavaScript("JSON.stringify(window.__probeState__ ?? null)") { value, _ in
                    if self.reported { return }
                    let probeState = (value as? String) ?? "null"
                    let lifecycle = self.lifecycleRecorder.summary()
                    self.write(status: "error", detail: "no pushed result after didFinish: \(detail); probeState=\(probeState); lifecycle=\(lifecycle)")
                }
            }
        }
    }
    ...
    func webView(
        ...
        lifecycleRecorder.mark("mediaPermission:type=\(type.rawValue):host=\(origin.host)")
    ...
    private func write(status: String, detail: String) {
        ...
        reported = true
        resultWriter.write(fileName: "wkwebview-getusermedia-timer-probe.json", status: status, detail: detail)
    }
}
...
private final class LifecycleRecorder {
    ...
    init() {
        ...
        center.addObserver(forName: UIApplication.didBecomeActiveNotification, object: nil, queue: .main) { [weak self] _ in
            self?.mark("didBecomeActive")
        }
        center.addObserver(forName: UIApplication.willResignActiveNotification, object: nil, queue: .main) { [weak self] _ in
            self?.mark("willResignActive")
        }
        center.addObserver(forName: UIApplication.didEnterBackgroundNotification, object: nil, queue: .main) { [weak self] _ in
            self?.mark("didEnterBackground")
        }
        center.addObserver(forName: UIApplication.willEnterForegroundNotification, object: nil, queue: .main) { [weak self] _ in
            self?.mark("willEnterForeground")
        }
    }

    func mark(_ label: String) {
        let delta = Int(Date().timeIntervalSince(startedAt) * 1000)
        marks.append("\(delta):\(label)")
    }
